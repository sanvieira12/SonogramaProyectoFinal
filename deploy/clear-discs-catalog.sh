...
echo "Alcance: discos con procedencia='VINYL_FUTURE' sin ventas, detalles de venta ni reservas."
...
SELECT d.id_disco
FROM disco d
WHERE d.procedencia = 'VINYL_FUTURE'
...
SELECT 'discos_vinylfuture_a_borrar' AS item, count(*) AS cantidad FROM target_discos;
SELECT 'discos_vinylfuture_preservados_por_referencias' AS item, count(*) AS cantidad FROM skipped_discos;
